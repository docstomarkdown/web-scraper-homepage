...
const features = [
    ...
    },
    {
        ...
        iconBg: "bg-emerald-50 dark:bg-emerald-950/40 border-emerald-100/50",
...
export default function Features() {
    return (
        <section id="features" className="pt-20 pb-12 bg-gradient-to-b from-slate-50 dark:from-slate-900/60 to-white dark:to-slate-950 relative overflow-hidden">
            <div className="absolute inset-0 pointer-events-none overflow-hidden">
                <div className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-slate-200 to-transparent" />
                <div className="absolute -top-40 -right-40 w-80 h-80 bg-blue-50/60 dark:bg-[#2772ED]/10 rounded-full blur-[100px]" />
                <div className="absolute -bottom-20 -left-20 w-60 h-60 bg-indigo-50/60 dark:bg-indigo-500/10 rounded-full blur-[80px]" />
            </div>
            ...
            <div className="max-w-7xl mx-auto px-6 md:px-12 relative">
                ...
                <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-60px" }} variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
                    {features.map((f, i) => {
                        const Icon = f.icon;
                        return (
                            <motion.div key={i} variants={fadeUpVariant} className={`group relative p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 ring-2 ring-transparent transition-all duration-300 overflow-hidden cursor-default -translate-y-0 hover:-translate-y-2 hover:border-slate-200 hover:shadow-2xl ${f.glow} ${f.ring}`}>
                                ...
                                <h3 className="relative z-10 text-xl font-bold text-slate-700 dark:text-slate-200 mb-3 group-hover:text-slate-900 transition-colors">{f.title}</h3>
                                <p className="relative z-10 text-slate-500 dark:text-slate-400 leading-relaxed [&>strong]:text-slate-500 [&>strong]:font-semibold">{f.description}</p>
