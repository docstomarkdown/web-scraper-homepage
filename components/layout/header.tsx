  ...
  return (
    <header
      className={cn(
        ...
        scrolled
          ? "bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-slate-200/60 dark:border-slate-800/60 shadow-sm py-3"
          : isHome
            ? "bg-transparent py-4"
        ...
        <div className="flex items-center justify-between">
            ...
            {navigationConfig.mainNav.map((item: any) => {
                ...
                return (
                  <DropdownMenu key={item.title}>
                    ...
                    <DropdownMenuContent
                      ...
                      sideOffset={20}
                      className={cn(
                        "p-6 shadow-2xl border-slate-100 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl ring-1 ring-slate-900/5 rounded-3xl animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 duration-200",
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className="px-4 py-2 text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-slate-800 rounded-full transition-all duration-200"
                >
                  {item.title}
                </Link>
              );
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:block">
              <Button
                asChild
                className="h-10 px-6 rounded-xl border border-[#2772ED]/25 bg-[#2772ED]/8 text-[#1f5ec2] dark:text-[#7aa7ff] hover:bg-[#2772ED]/14 hover:border-[#2772ED]/35 shadow-sm shadow-[#2772ED]/10 transition-all duration-300"
              >
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="w-[300px] sm:w-[400px] p-0 border-l border-slate-100 dark:border-slate-800">
                <SheetHeader className="p-6 border-b border-slate-100 dark:border-slate-800 text-left">
                  <SheetTitle className="flex items-center gap-2">
                    <Icons.logo className="w-8 h-8" />
                    <span className="font-bold text-lg text-slate-900 dark:text-white">Web Scraper Pro</span>
                  </SheetTitle>
                </SheetHeader>
                <div className="overflow-y-auto h-[calc(100vh-80px)] p-6 pb-20">
                  <div className="flex flex-col gap-6">
                    {navigationConfig.mainNav.map((item: any, i: number) => (
                      <div key={i}>
                        {item.grouped ? (
                            ...
                            <div className="pl-4 space-y-6">
                              {item.groups.map((group: any) => (
                                <div key={group.title} className="space-y-3">
                                  ...
                                  <div className="pl-3.5 border-l border-slate-100 dark:border-slate-800 space-y-2">
                                    {group.items.map((subItem: any) => (
                                      <Link
                                        key={subItem.href}
                                        href={subItem.href}
                                        className="block pl-4 py-1 text-sm text-slate-500 dark:text-slate-400 hover:text-blue-600"
                                      >
                                        {subItem.title}
                                      </Link>
                                    ))}
                          ...
                          <Link
                            href={item.href}
                            className="block font-semibold text-slate-900 dark:text-white hover:text-blue-600 text-lg"
                          >
                            {item.title}
                          </Link>
                        )}
                    ...
                    <div className="pt-6 mt-auto">
                      <Button asChild className="w-full h-11 rounded-xl border border-[#2772ED]/25 bg-[#2772ED]/8 hover:bg-[#2772ED]/14 hover:border-[#2772ED]/35 text-base text-[#1f5ec2] dark:text-[#7aa7ff] shadow-sm shadow-[#2772ED]/10 transition-all duration-300">
